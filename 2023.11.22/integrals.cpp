...
int nfp, double **fc, double **fc_zeta, double **fc_zetazeta, int num_variables, double **triangles_uv) {
    ...
    printf("calculating integral of (R, Z)*nu_i...\n");
    fflush(stdout);
    int num_triangles_in_plane = (int) triangles_uv[0][6];
    int num = num_variables;
    ...
    for (int i = 0; i < num_sections; i++) {
        ...
        for (int j = 0; j < num_triangles_in_plane; j++) {
            for (int k = 0; k < 18; k++) {
                functional_nu_sec_tri_18_num[i][j][k] = numerical_integration_RZ_over_triangle(triangles_uv[j][0], triangles_uv[j][1], triangles_uv[j][2], functional_nu, \
