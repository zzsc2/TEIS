...
double *FuncPoisson(double u, double v, double ***R, double ***Z, double ***Rzeta, double ***Zzeta, double ***Rzetazeta, double ***Zzetazeta, double ***G, int position_sec, int position_tri) {
    ...
    for (int i = 1; i < 10; i++) {
        var1[i] = D * (p[0][i-1] + R_*p[2][i-1] + R_*p[4][i-1] + 1.0/R_*p[7][i-1]);
    }
    // var1[10] = D * R_;
...
double ***FuncBeltrami(double u, double v, double ***R, double ***Z, double ***Rzeta, double ***Zzeta, double ***Rzetazeta, double ***Zzetazeta, double ***G, int position_sec, int position_tri, int num_scalar, double mu) {
    ...
    // var1[0][1][2] += (1.0 / R_) * D * R_;
    // //
    // var1[1][0][1] += (  p[8][0] / R_) * D * R_;
    // var1[1][0][2] += (- p[1][0]) * D * R_;

    // var1[1][1][0] += (- p[8][0] / R_) * D * R_;
    // var1[1][1][2] += (  p[0][0]) * D * R_;

    // var1[1][2][0] += (  p[1][0]) * D * R_;
    // var1[1][2][1] += (- p[0][0]) * D * R_;
    // //
    // var1[2][0][1] += (  p[8][1] / R_) * D * R_;
    // var1[2][0][2] += (- p[1][1]) * D * R_;

    // var1[2][1][0] += (- p[8][1] / R_) * D * R_;
    // var1[2][1][2] += (  p[0][1]) * D * R_;

    // var1[2][2][0] += (  p[1][1]) * D * R_;
    // var1[2][2][1] += (- p[0][1]) * D * R_;
    // //
    // var1[9][0][1] += (  p[8][8] / R_) * D * R_;
    // var1[9][0][2] += (- p[1][8]) * D * R_;

    // var1[9][1][0] += (- p[8][8] / R_) * D * R_;
    // var1[9][1][2] += (  p[0][8]) * D * R_;

    // var1[9][2][0] += (  p[1][8]) * D * R_;
    // var1[9][2][1] += (- p[0][8]) * D * R_;
    // //
    // var1[0][0][0] += (- mu) * D * R_;
    // var1[0][1][1] += (- mu) * D * R_;
    // var1[0][2][2] += (- mu) * D * R_;    
...
double *****MG_tri_6_18_18_18(double ***G, double **triangles_uv) {
    ...
    for (int i = 0; i < num_triangles_in_plane; i++) {
        ...
        for (int l = 0; l < 20; l++) {
            for (int s = 0; s < 3; s++) {
                for (int t = 0; t < 3; t++) {
                    if ((m_order[l] - s >= 0) && (n_order[l] - t >= 0)) {
                        #pragma omp parallel for collapse(2)
                        for (int j = 0; j < 20; j++) {
                            for (int k = 0; k < 20; k++) {
...
double ***solve_magnetic_field_sparse_ALM_div(double**** divergence_tri_3_var_modes, double ***M_tri_18Nn_18Nn, double **B_tri_18Nn, int **points_sequence, double **boundary_DoFs, int num_modes, int num_triangels_in_plane, int nfp_ft, int num_scalar, int flag, double error, time_t time_s) {
    ...
    if (flag == 0) {
        ...
        num_boundary_DoFs = num_edges * num_modes_normal + 2;
    }
    else {
                        ...
                        for (int t = 0; t < 2*num_modes-1; t++) { // coeff
                            ...
                            for (int k = 0; k < 3; k++) {
                                for (int l = 0; l < 2; l++) {
                                    ...
                                    coeff_L.push_back( T(index2 + p1[l], index3 + s * num_scalar + k, boundary_DoFs[k][index1 + t] * p2[l]) );
                                }
                            }
                            delete [] p1;
                            delete [] p2;
...
double ***Func_SIME_magnetic_field(double u, double v, double ***R, double ***Z, double ***Rzeta, double ***Zzeta, double ***mf_3_tri_18, double **pres_tri_18, double ***G, int num_triangles, int num_modes, int nfp_ft, int position_sec, int position_tri, int num_scalar) {
    ...
    double D, E, H;
    double uv[20], uv_u[20], uv_v[20];
    double R_ = 0.0, R_u = 0.0, R_v = 0.0, R_zeta = 0.0;
    double Z_ = 0.0, Z_u = 0.0, Z_v = 0.0, Z_zeta = 0.0;
    double Bphi = 0.0, BR = 0.0, BZ = 0.0, pres = 0.0;
    ...
    for (int i = 0; i < 20; i++) {
        ...
        for (int j = 0; j < 18; j++) {
            ...
            BR +=         G[position_tri][i][j] * uv[i] *    mf_3_tri_18[0][position_tri][j];
            BZ +=         G[position_tri][i][j] * uv[i] *    mf_3_tri_18[1][position_tri][j];
            Bphi +=       G[position_tri][i][j] * uv[i] *    mf_3_tri_18[2][position_tri][j];
            pres +=       G[position_tri][i][j] * uv[i] *    pres_tri_18[position_tri][j];       
        }
    }
    ...
    // num_scalar = 3 for magnetic field
    ...
    var1[0][0][2] = (Bphi / R_) * D * R_;
    var1[0][2][2] = (- BR / R_) * D * R_;
    for (int i = 1; i < 4; i++) {
       var1[i][0][0] = (- Bphi / R_ * p[2][i-1] - BZ * p[1][i-1]) * D * R_;
       var1[i][0][1] = (BZ * p[0][i-1]) * D * R_;
       var1[i][0][2] = (Bphi * p[0][i-1]) * D * R_;

       var1[i][1][0] = (BR * p[1][i-1]) * D * R_;
       var1[i][1][1] = (- BR * p[0][i-1] - Bphi / R_ * p[2][i-1]) * D * R_;
       var1[i][1][2] = (Bphi * p[1][i-1]) * D * R_;

       var1[i][2][0] = (BR / R_ * p[2][i-1]) * D * R_;
       var1[i][2][1] = (BZ / R_ * p[2][i-1]) * D * R_;
       var1[i][2][2] = (- BZ * p[1][i-1] - BR * p[0][i-1]) * D * R_;

    //    var1[4][0][0] += (mu0 * p[0][i-1]) * D * R_;
    //    var1[4][1][0] += (mu0 * p[1][i-1]) * D * R_;
    //    var1[4][2][0] += (mu0 * p[2][i-1] / R_) * D * R_;
    }
    // var1[4][0][0] += (mu0 / R_) * D * R_;
